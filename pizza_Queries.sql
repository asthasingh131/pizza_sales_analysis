create database if not exists pizzahutDB;
...
(select round(sum(order_details.quantity * pizzas.price), 2)
 ...
 from order_details
 join pizzas on pizzas.pizza_id = order_details.pizza_id )  * 100,0) as revenue
...
-- Note: both the queries (12 and 13) runs fine in MySQL 8.x+
-- Some SQL editors may show a red cross warning due to window functions.